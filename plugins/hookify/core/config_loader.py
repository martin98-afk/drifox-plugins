...
def load_rules(event: Optional[str] = None, search_dirs: Optional[List[str]] = None) -> List[Rule]:
    """Load all hookify rules.

    Search order:
      1. Plugin's own rules/ directory (primary, .drifox/plugins/hookify/rules/)
      2. {CWD}/.claude/ (backward compat)
      3. {project_root}/.claude/ (backward compat)

    Args:
        event: Optional event filter ("bash", "file", "stop", etc.)
        search_dirs: Deprecated, kept for backward compatibility.
    ...
    # 1. 插件自身的 rules/ 目录（首要）
    _plugin_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # .drifox/plugins/hookify
    _rules_dir = os.path.join(_plugin_root, 'rules')
    if os.path.isdir(_rules_dir):
        search_bases.append(_rules_dir)
        seen_bases.add(_rules_dir)

    # 2. 兼容：从 search_dirs 参数或其默认值推算 .claude/
    if search_dirs is None:
        search_dirs = [os.getcwd()]
        # 从 config_loader.py 位置推算项目根目录
        _loader_dir = os.path.dirname(os.path.abspath(__file__))
        for _ in range(4):
            _loader_dir = os.path.dirname(_loader_dir)
        if _loader_dir not in search_dirs:
            search_dirs.append(_loader_dir)

    for _base in search_dirs:
        _dot_claude = os.path.join(_base, '.claude')
        if os.path.isdir(_dot_claude) and _dot_claude not in seen_bases:
            search_bases.append(_dot_claude)
            seen_bases.add(_dot_claude)

    # ── 遍历搜索目录找 hookify.*.local.md ──
    files = []
    for _base in search_bases:
        _pattern = os.path.join(_base, 'hookify.*.local.md')
