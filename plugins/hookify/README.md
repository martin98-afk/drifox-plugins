...
### Drifox
Place the plugin in `~/.drifox/plugins/hookify/` (or symlink it). Drifox's PluginManager auto-discovers plugins with `.drifox-plugin/plugin.json`.

**No additional configuration needed** — the hookify hooks, commands, agents, and skills are automatically loaded by Drifox. See the commands section above for usage.
