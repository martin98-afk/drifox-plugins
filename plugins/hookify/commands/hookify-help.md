...
These hooks read configuration files from `.drifox/plugins/hookify/rules/hookify.*.local.md` and check if any rules match the current operation.

### 2. Configuration Files

Users create rules in `.drifox/plugins/hookify/rules/hookify.{rule-name}.local.md` files:
...
**Rule Files**: Keep rules in `.drifox/plugins/hookify/rules/hookify.*.local.md` - they should be git-ignored.
...
**Hook not triggering:**
- Check rule file is in `.drifox/plugins/hookify/rules/` directory
...
4. Refine the rule by editing `.drifox/plugins/hookify/rules/hookify.warn-rm.local.md`

5. Create more rules as you encounter unwanted behaviors

For more examples, check the plugin's `examples/` directory.
