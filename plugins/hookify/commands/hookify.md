...
For each confirmed behavior, create a `hookify.{rule-name}.local.md` file in the hookify plugin's `rules/` directory:

**路径**: `.drifox/plugins/hookify/rules/hookify.{rule-name}.local.md`
...
Rule files go in the hookify plugin's `rules/` directory:

```
.drifox/plugins/hookify/rules/hookify.{name}.local.md
```

1. Ensure the `rules/` directory exists:
   - Path: `.drifox/plugins/hookify/rules/`
   - Create it if missing: `mkdir -p .drifox/plugins/hookify/rules`

2. Use Write tool to create each `hookify.{name}.local.md` file inside that directory

3. Show user what was created:
   ```
   Created 3 hookify rules:
   - .drifox/plugins/hookify/rules/hookify.dangerous-rm.local.md
   - .drifox/plugins/hookify/rules/hookify.console-log.local.md
   - .drifox/plugins/hookify/rules/hookify.sensitive-files.local.md
...
4. Create `.drifox/plugins/hookify/rules/hookify.dangerous-rm.local.md`:
   ```markdown
   ---
...
- **File location**: Create files in `.drifox/plugins/hookify/rules/` directory
...
1. Check the path: `.drifox/plugins/hookify/rules/hookify.{name}.local.md`
2. Ensure the `rules/` directory exists: `ls .drifox/plugins/hookify/rules/`
3. Verify file was created with Glob or ls

**If rule doesn't trigger after creation:**
1. Verify file is in `.drifox/plugins/hookify/rules/` directory
