...
1. Use Glob tool to find all hookify rule files:
   ```
   pattern: ".drifox/plugins/hookify/rules/hookify.*.local.md"
   ```
...
**Status**: ✅ Active
**File**: .drifox/plugins/hookify/rules/hookify.dangerous-rm.local.md
```
...
2. Or manually create `.drifox/plugins/hookify/rules/hookify.my-rule.local.md` files
...
Check the plugin's `examples/` directory for example rule files.
```
