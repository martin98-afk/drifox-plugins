...
- 🖱 手动换 IP 按钮
- ⚙️ 配置存插件自身数据目录（不随云端备份，避免污染 user-custom 与频繁上传）
...
## 配置项（.drifox/plugins/ip-switcher/data/ip-switcher.json）
...
> 旧版本数据存于 `.drifox/plugins/user-custom/ip-switcher/`，首次运行新版本会自动迁移到
> `.drifox/plugins/ip-switcher/data/` 并删除旧目录，无需手动处理。
