...
description: 生成横向翻页网页 PPT（单 HTML 文件），含 WebGL 背景、章节幕封、数据大字报、图片网格等模板 + MiniMax TTS 逐页语音配音。提供两种风格：① "电子杂志 × 电子墨水"（衬线 + 流体背景 + 暖色） ② "瑞士国际主义"（无衬线 + 网格点阵 + IKB/柠檬黄/柠檬绿/安全橙高亮）。当用户需要制作分享/演讲/发布会风格的网页 PPT，或提到"杂志风 PPT"、"瑞士风 PPT"、"Swiss Style"、"horizontal swipe deck"、"语音 PPT"、"配音"、"TTS"、"带讲解的PPT"时使用。
---
...
| `ESC` | 索引视图 |
| `F` | 切换浏览器全屏 |
| `H` | 切换底部 UI 显示/隐藏（导航点、语音条、提示文字） |
| `Home` / `End` | 首尾页 |

### 语音配音（新增能力）

在生成 PPT 后可基于 MiniMax TTS 为每页添加语音讲解：
- 自动从演讲稿提取每页文本 → 调用 MiniMax TTS API → 生成 `audio/pXX.mp3`
- HTML 底部自动集成音频控制条（播放/暂停/进度条）
- 翻页自动播放对应音频 + 播完自动翻下一页
- 音色可选：男声清晰 / 男声播音 / 女声柔和 / 女声知性
- API Key 通过环境变量 `MINIMAX_API_KEY` 传入，绝不硬编码
...
- ⭐ 需要**自动语音讲解**的培训 / 展览 / 自助浏览 PPT
- ⭐ 希望观众**扫码后自行翻页听讲解**的展板型 PPT
...
18. **保留快捷键**——右下角必须提示 `B 静态 · F 全屏 · H 隐藏UI`:
    - `F`: 切换浏览器全屏(`requestFullscreen`/`exitFullscreen`)
    - `H`: 切换底部 UI 显示/隐藏(添加/移除 `body.ui-hidden`;CSS 用 `body.ui-hidden #nav, body.ui-hidden #audio-control, body.ui-hidden #hint{display:none}`)
    - `B`: 切换低功耗模式,停止 WebGL/ASCII canvas RAF 和 Motion 入场动画
...
### Step 7 · 生成语音讲解（MiniMax TTS）⭐ 新增

在 PPT 内容定稿后，可为每页添加语音讲解。**先确认用户是否需要此功能**，如果不需要则跳过。

#### 7.1 · 前置准备

```bash
# 设置 MiniMax API Key（从环境变量读取，绝不硬编码）
set MINIMAX_API_KEY=your_key_here
```

API Key 获取：在 [platform.minimax.com](https://platform.minimax.com) 注册 → 账户管理 → API Keys → 创建。

#### 7.2 · 输出结构（在 ppt/ 目录下额外生成）

```
ppt/
├── index.html              ← 主演示文件（已含音频播放 JS）
├── 演讲稿.md               ← 每页演讲稿（已有）
├── audio/                  ← 新增：30 个 MP3 语音文件
│   ├── p01.mp3
│   ├── p02.mp3
│   └── ...
└── scripts/
    └── generate_tts.py     ← 新增：TTS 合成脚本（本 skill 附带）
```

#### 7.3 · 演讲稿格式标准

为让 TTS 脚本正确解析，演讲稿必须按以下格式编写：
...
关键规则：
- 页号用 `P01` `P02` ... `P30` 格式（两位数补零）
- `**演讲词**` 标记后跟正文（支持 `>` 引用标记包裹）
- 每页文本长度建议 200~600 字（API 限制 < 10,000 字）

#### 7.4 · 生成命令

```bash
# 生成全部 30 页（脚本在本 skill 的 scripts/ 目录下）
...
# 单页重试
python scripts/generate_tts.py --input ppt/演讲稿.md --output-dir ppt/audio --page P05
```

#### 7.5 · 集成到 HTML

执行以下步骤将音频控制集成到 `index.html`：

**① 添加音频控制条 HTML**（放在 `<div id="nav"></div>` 后面）：
...
#audio-control:hover{background:rgba(0,0,0,.1)}
body.dark-bg #audio-control{background:rgba(255,255,255,.08);color:var(--paper)}
body.dark-bg #audio-control:hover{background:rgba(255,255,255,.14)}
#audio-play-btn{background:none;border:0;cursor:pointer;font-size:16px;padding:2px 4px;color:var(--ink);opacity:.7;transition:opacity .2s;line-height:1}
#audio-play-btn:hover{opacity:1}
body.dark-bg #audio-play-btn{color:var(--paper)}
#audio-play-btn.playing{opacity:1}
#audio-status{white-space:nowrap;letter-spacing:.04em}
...
#audio-progress-bar{height:100%;width:0%;background:var(--accent);border-radius:2px;transition:width .3s linear}
#audio-control.loading #audio-play-btn{opacity:.4;pointer-events:none}
```

**③ 在 `go()` 函数末尾添加音频触发**：

```javascript
// 在 go() 函数内部，lock 解除之前添加：
setTimeout(()=>playAudioForSlide(idx), 300);
```

**④ 添加音频管理 JS**（放在 `go()` 函数和 ESC 索引视图之间）：
...
function initAudio(){
  ...
  audioEl.addEventListener('ended', ()=>{
    audioPlaying = false; updateAudioUI(false);
  ...
  audioEl.addEventListener('canplay', ()=>{
    audioLoaded = true;
    document.getElementById('audio-control').classList.remove('loading');
  });
  audioEl.addEventListener('error', ()=>{
    document.getElementById('audio-control').classList.remove('loading');
    document.getElementById('audio-status').textContent = '音频不可用';
  });
}

function getAudioPath(slideIdx){
  const num = (slideIdx + 1).toString().padStart(2, '0');
  return AUDIO_DIR + 'p' + num + '.mp3';
}

function playAudioForSlide(slideIdx){
  if(!audioEl) return;
  audioEl.pause(); audioPlaying = false;
  document.getElementById('audio-progress-bar').style.width = '0%';
  audioEl.src = getAudioPath(slideIdx);
  audioEl.load();
  audioLoaded = false;
  document.getElementById('audio-control').classList.add('loading');
  document.getElementById('audio-status').textContent = '加载中...';
  const playPromise = audioEl.play();
  if(playPromise !== undefined){
    playPromise.then(()=>{
      audioPlaying = true; audioLoaded = true;
      document.getElementById('audio-control').classList.remove('loading');
      document.getElementById('audio-status').textContent = '🔊 语音讲解';
      updateAudioUI(true);
    }).catch(()=>{
      audioPlaying = false; audioLoaded = true;
      document.getElementById('audio-control').classList.remove('loading');
      document.getElementById('audio-status').textContent = '▶ 点击播放讲解';
      updateAudioUI(false);
    });
  }
}
...
function toggleAudioPlay(){
  ...
  if(audioPlaying){
    ...
    audioEl.play().then(()=>{
      audioPlaying = true; updateAudioUI(true);
      document.getElementById('audio-status').textContent = '🔊 语音讲解';
    }).catch(()=>{});
  }
}
...
document.getElementById('audio-play-btn').addEventListener('click', (e)=>{
  e.stopPropagation(); toggleAudioPlay();
});

// 低功耗模式时暂停音频
addEventListener('swiss-low-power-change', e=>{
  if(e.detail.on && audioPlaying) toggleAudioPlay();
});
```

**⑤ 验证**：
- 打开 `index.html`，首次打开因浏览器策略不会自动播放，点击 ▶ 即可
- 之后每翻一页自动播放对应音频
- 音频播完自动翻至下一页
- 暗色/亮色主题下控制条自动适配
- 按 B 键低功耗模式自动暂停
...
│   └── generate_tts.py       ← ⭐ MiniMax TTS 语音合成脚本（新增）
└── references/
...
9. ⭐ **如果需要语音讲解**:先读 `references/audio.md` 了解集成方式,然后用 `scripts/generate_tts.py` 合成音频
