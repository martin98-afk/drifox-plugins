---
description: 中国地图合规守则：任何地图生成/可视化/路线/位置服务请求必须加载本技能，强制执行合规红线——禁用 Google/Apple/海外版 Bing/直接使用海外瓦片的 OpenStreetMap 等数据源、API key 安全、领土完整性要求、隐私保护，并附腾讯地图 GL JS 各场景接入规范。
