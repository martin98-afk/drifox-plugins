...
> 🦊 **DriFox 插件** — 通过插件市场安装。`skills.knowledge-base-skills`
...
```
knowledge-base-skills/                  ← DriFox 插件根目录
├── .drifox-plugin/plugin.json          插件清单 (name: knowledge-base-skills)
├── skills/kb-retriever/                ← 技能根目录 (name: kb-retriever)
│   ├── SKILL.md                         主技能
│   ├── references/
│   │   ├── pdf_reading.md               PDF 处理指南
│   │   ├── excel_reading.md             pandas 读取 Excel 的方法
│   │   └── excel_analysis.md            Excel 的过滤 / 聚合 / 派生指标方法
│   └── scripts/
│       └── convert_pdf_to_images.py     文本抽取失败时把 PDF 转图像的兜底脚本
