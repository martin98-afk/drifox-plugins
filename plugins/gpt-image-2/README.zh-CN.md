...
[English](./README.md) · [DriFox 插件](../../README.zh-CN.md#插件)

> 🦊 **DriFox 插件** — 通过插件市场安装。`skills.gpt-image-2`
...
## Skill 结构（DriFox 插件格式）

```
gpt-image-2/                         ← DriFox 插件根目录
├── .drifox-plugin/plugin.json      插件清单 (name: gpt-image-2)
├── skills/gpt-image-2/             ← 技能根目录
│   ├── SKILL.md                     主技能定义
│   ├── scripts/
│   │   ├── check-mode.js            模式 A/B/C 探测器
│   │   ├── generate.js              文本生图（仅 Mode A）
│   │   ├── edit.js                  图像编辑（仅 Mode A）
│   │   ├── shared.js                共享逻辑
│   │   └── package.json
│   └── references/
│       ├── prompt-writing.md        方法论
│       ├── ui-mockups/              UI 样机
│       ├── product-visuals/         产品视觉
│       ├── infographics/            信息图
│       ├── poster-and-campaigns/    海报与 Campaign
│       ├── slides-and-visual-docs/  幻灯片
│       ├── portraits-and-characters/ 肖像与角色
│       ├── scenes-and-illustrations/ 场景与插画
│       ├── editing-workflows/       编辑工作流
│       ├── avatars-and-profile/     头像与个人形象
│       ├── storyboards-and-sequences/ 故事板
│       ├── grids-and-collages/      网格与拼贴
│       ├── branding-and-packaging/  品牌与包装
│       ├── typography-and-text-layout/ 版式
│       ├── assets-and-props/        素材
│       ├── academic-figures/        学术图
│       ├── technical-diagrams/      技术图
│       └── maps/                    地图
